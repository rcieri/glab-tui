...
pub struct App {
    ...
    /// Mapping from the `all_items` display strings to the underlying value
    /// for the active `switch_repo` selector. Lets the overlay render short
    /// names (basename or path relative to `repos_dir`) while the submit
    /// handler still gets the absolute path it needs for `set_current_dir`.
    /// Cleared whenever `selector` is dropped.
