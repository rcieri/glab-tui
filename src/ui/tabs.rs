...
use crate::utils::format::{format_duration, format_ref, render_markdown, time_ago, truncate};
use ratatui::{
    Frame,
...
pub(crate) fn render_tab_pipelines(
    ...
    if app.pipelines.items.is_empty() && app.loading_tabs.contains(&app.active_tab) {
        ...
        let rows = filtered_pipelines.iter().enumerate().map(|(idx, p)| {
            ...
            if app.is_column_visible(Tab::Pipelines, "Workflow") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &truncate(p.name(), 30),
                    &app.search_query,
                    is_row_highlighted,
            ...
            if app.is_column_visible(Tab::Pipelines, "Title") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &truncate(p.display_title(), 40),
                    &app.search_query,
                    is_row_highlighted,
                    is_checked,
                    Style::default().fg(THEME.read().unwrap().text_normal),
            ...
            if app.is_column_visible(Tab::Pipelines, "Branch") {
                row_cells.push(super::helpers::render_fuzzy_cell(
            ...
            if app.is_column_visible(Tab::Pipelines, "Created") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &time_ago(p.created_at().unwrap_or("")),
                    &app.search_query,
                    is_row_highlighted,
                    is_checked,
                    Style::default().fg(THEME.read().unwrap().text_muted),
        ...
        if let Some(selected) = app.pipelines.state.selected() {
            if let Some(p) = filtered_pipelines.get(selected) {
                let pipeline_jobs = app.pipeline_jobs.get(&p.id());
                ...
                if is_github {
                    ...
                    text.push(Line::from(vec![
                        ...
                        Span::styled(
                            p.display_title().to_owned(),
                            Style::default().fg(theme.text_normal),
                        ),
                    ]));
                    text.push(Line::from(vec![
                        Span::styled("Run ID:   ", Style::default().fg(theme.text_muted)),
                        Span::styled(
                            format!("#{}", p.id()),
                    ...
                    text.push(Line::from(vec![
                        Span::styled("Event:    ", Style::default().fg(theme.text_muted)),
                        Span::styled(p.event().to_owned(), Style::default().fg(theme.text_normal)),
                    ]));
                    text.push(Line::from(vec![
                    ...
                    text.push(Line::from(vec![
                        Span::styled("SHA:      ", Style::default().fg(theme.text_muted)),
                        Span::styled(
                            p.head_sha().to_owned(),
                            Style::default().fg(theme.text_muted),
                        ),
                    ...
                    text.push(Line::from(vec![
                        ...
                        Span::styled(
                            format_duration(p.duration_seconds()),
                            Style::default().fg(theme.text_normal),
                        ),
                    ]));
                    if let Some(jobs) = pipeline_jobs {
                        ...
                        text.push(Line::from(vec![Span::styled(
                            format!(
                                "Jobs: {} total, {} passed, {} failed, {} running",
    ...
    if app.jobs.items.is_empty() && app.loading_tabs.contains(&app.active_tab) {
        ...
        let rows = filtered_jobs.iter().enumerate().map(|(i, j)| {
            ...
            if app.is_column_visible(Tab::Jobs, "Duration") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &format_duration(j.duration_seconds()),
                    &app.search_query,
                    is_job_selected,
                    is_checked,
                    Style::default().fg(THEME.read().unwrap().text_muted),
            ...
            if app.is_column_visible(Tab::Jobs, "Runner") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &truncate(j.runner().unwrap_or("\u{2014}"), 20),
                    &app.search_query,
                    is_job_selected,
                    is_checked,
                    Style::default().fg(THEME.read().unwrap().text_normal),
            ...
            if app.is_column_visible(Tab::Jobs, "Tags") {
                row_cells.push(super::helpers::render_fuzzy_cell(
                    &truncate(
                        &j.tags()
                            .map(|t| t.join(", "))
                ...
                if let Some(selected) = app.jobs.state.selected() {
                    if let Some(j) = filtered_jobs.get(selected) {
                        ...
                        text.push(Line::from(vec![
                            ...
                            Span::styled(
                                j.runner().unwrap_or("\u{2014}").to_string(),
                                Style::default().fg(theme.text_normal),
                            ),
                        ]));
                        ...
                        if let Some(steps) = j.steps() {
                            if !steps.is_empty() {
                                ...
                                for step in steps {
                                    let step_status = if step.status == "completed" {
                                        step.conclusion
                                            .clone()
                                    ...
                                    let step_color = match step_status.as_str() {
                                        "success" => theme.green,
                                        "failure" => theme.red,
                                        "cancelled" | "canceled" => theme.text_muted,
                                    ...
                                    text.push(Line::from(vec![
                                        ...
                                        Span::styled(
                                            format!("{:>10}", step_status.to_uppercase()),
